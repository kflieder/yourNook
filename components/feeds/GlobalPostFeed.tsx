...
function GlobalPostFeed({ currentUser }: GlobalPostFeedProps) {
  ...
  return (
    <div className="grid grid-cols-5 justify-between h-screen pt-30">
      <div className="col-span-3 flex flex-col gap-y-4 px-4 overflow-scroll overflow-x-hidden">
        <h1>Latest Posts</h1>
        {posts.map((post) => (
          <div key={post.id} className="w-full border flex justify-center">
            <LivePost
              width="w-96"
              post={post}
      ...
      <div className="flex flex-col col-span-2 gap-y-4 px-2 pt-10 overflow-scroll overflow-x-hidden">
        <DMComponent
          currentUser={currentUser.uid}
