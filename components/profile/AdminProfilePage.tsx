...
function ProfilePage({ userData, posts }: ProfilePageProps) {
  ...
  const [activeAdminTab, setActiveAdminTab] = useState<"adminDashboard" | "profile">("adminDashboard");
  ...
  function handleAdminTabChange(tab: "adminDashboard" | "profile") {
  ...
  return (
    <>
    {
      userData.isAdmin && isOwner ? (
        ...
        <h1 className='cursor-pointer' onClick={() => setActiveAdminTab("profile")}>Profile</h1>
        </div>
        {activeAdminTab === "adminDashboard" && adminDashboard}
