...
function FriendsList({ currentUserUid }: FriendsListProps) {
    ...
    const [mutualUserData, setMutualUserData] = React.useState<{displayName: string; uniqueUrl: string; profilePicture: string}[]>([]);
    ...
    useEffect(() => {
        ...
        const mutuals: string[] = followers.filter((follower: string) => following.includes(follower));
        ...
        async function fetchMutualUserData() {
            ...
            console.log("Mutual User Data:", mutualUserData);
    ...
    <div className='flex flex-col border p-4 mr-4 text-center' >
      ...
      <ul className="border flex flex-col items-center space-y-2 py-4">
        {mutualUserData.map((user, index) => (
            ...
            <Link className='flex items-center space-x-2' href={`/profile/${user.uniqueUrl}`} >
                <img src={user.profilePicture} alt={`${user.displayName}'s profile`} className="w-10 h-10 rounded-full" />
              <li className="cursor-pointer hover:underline">{user.displayName}</li>
            </Link>
            </div>
        ))}
      </ul>
